...
package com.travelapp.bustrainflightmobile.androidApp.payload

import com.travelapp.bustrainflightmobile.androidApp.model.data.Country
import com.travelapp.bustrainflightmobile.androidApp.model.data.Country.*
...
import com.travelapp.bustrainflightmobile.androidApp.model.data.TransportationType
import com.travelapp.bustrainflightmobile.androidApp.model.data.TransportationType.*
...
//    ),
    ...
    BLABLACAR(
        ...
        applicableLocations = emptySet(),
        affiliateUrls = mapOf(
            RIDE_SHARE to "https://www.blablacar.co.uk/"
        )
    );
    ...
    companion object {
        ...
        private fun Path.relate(locations: Set<Location>): Boolean {
            locations.forEach {
                if (this.from.name == it.name || this.to.name == it.name) return true
            }
            return false
        }
    }
